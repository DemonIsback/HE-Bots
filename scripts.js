...
var labels = {
	...
	researchSotware: [:Research software:]
}
...
//This function implements the research software algorithm 
function executeResearchSoftware(request){
	      var softwareId;
        switch(request.stepId){
                case "checkSoftware":
                if (getSoftwareId(request.software.split(",")[0],request.software.split(",")[1]) != null){
			          ...
			          break;
        case "goToMySoftwarePanel":
			          setNextStep(request, "researchSoftware");
			          goToPage("/university");
			          
			          break;
			  case "researchSoftware":
			          setNextStep(request, "waitForInstallSoftwareProcess");
			          softwareId = getSoftwareId(request.software.split(",")[0],request.software.split(",")[1]);
			          if (softwareId != null){
				                goToPage("/university?view=software&cmd=up&id=" + softwareId);
			          } else {
				                window.alert("Hey, it seems you don't have that software anymore :(");
				                setNextStep(request, "finishAll");
				                getSomeElement("a", "href", "?view=logout", 0).click(); //Click on the Home Button
				
			          }
			          break;
			  case "waitForResearchSoftwareProcess":
			          if (getSomeElement("div", "class", "alert alert-success", 0) == null){
			                	if (getSomeElement("div", "class", "alert alert-error", 0) == null){
					                      setNextStep(request, "goToLog-1");
				                } else {
					                      setNextStep(request, "finishCycle");
				                      	getSomeElement("a", "href", "?view=logout", 0).click(); //Click on the Software Button
				                }
			         } else {
				               setNextStep(request, "goToLog-1");
				               location.reload(true);
		   ...
		   case "waitForEditLogProcess-1":
			  ...
			  case "finishCycle":
			          if(request.ip.length > 0){
			       	          setNextStep(request, "goToHomePage");
				                goToPage("/home");
			          } else {
       ...
       case "finishAll":
			         request = resetRequestDada(request); 
			         request.job = defaultJob.researchSoftware; 
			         setNextStep(request, "chooseJob");
			         goToPage("/software");
			         break;
